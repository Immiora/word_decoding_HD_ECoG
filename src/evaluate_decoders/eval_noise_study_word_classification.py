...
python evaluate_decoders/eval_noise_study_word_classification.py \
    --task jip_janneke \
    --clf_type logreg \
    --subject_code xoop \
        ...
        for isubj, subject in enumerate(subjects):
            ...
            for imod, mod in enumerate(args.model):
                ...
                for fold in range(12):
                    ...
                    train_targets = apply_noise(train_targets, noise_sigma)
                    ...
                    train_input = apply_noise(train_input, noise_sigma)
                ...
                res.to_csv(op.join(trial_dir, 'eval_n_folds12_classify_' + args.clf_type + '_noise' + str(noise_sigma) + '.csv'))
                ...
                for i, pr in enumerate([prob, prob_brain]):
                    ...
                    x.to_csv(op.join(trial_dir, 'eval_n_folds12_classify_' + args.clf_type + input_type + '_noise' + str(noise_sigma) + 'prob.csv'))
                ...
                if plotdir is not None:
                    plt.savefig(os.path.join(plotdir, 'eval_classify_' + args.clf_type + '_noise' + str(noise_sigma) +
                                             '_confusion_matrix_recon_brain_sorted.pdf'),
