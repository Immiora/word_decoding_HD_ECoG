...
python stats/plot_distribution_best_params.py \
    --task jip_janneke \
...
def main(args):
    ...
    for subject in args.subject:
        for model in args.model:
            ...
            loss = loss.append(pd.DataFrame(min_loss))

    sns.swarmplot(x='loss', y='model', data=loss, hue='subject', orient='h')
    if args.plot_dir != '':
        plt.savefig(op.join(args.plot_dir, args.task, 'losses.pdf'), dpi=160, transparent=True)
        plt.close()
##
    ...
    for p in param_names:
        ...
        if args.plot_dir != '':
            plt.savefig(op.join(args.plot_dir, args.task, 'params_' + p + '.pdf'), dpi=160, transparent=True)
            plt.close()
